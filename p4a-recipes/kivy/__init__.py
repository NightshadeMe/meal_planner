...
class KivyRecipe(PyProjectRecipe):
    ...
    def get_recipe_env(self, arch, **kwargs):
        ...
        # Kivy 2.3.x's cgl_gl.c has a handful of GL function-pointer typedefs
        # that don't exactly match the stricter GLES headers shipped in NDK
        # 26+ (e.g. missing a `const` on a `char **` param). Older NDKs' clang
        # only warned about this; NDK 28c's clang treats it as a hard error.
        # Downgrade it back to a warning rather than patching every mismatch
        # in this generated file by hand.
